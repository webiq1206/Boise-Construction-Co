  ...
  async upsertUser(userData: UpsertUser): Promise<User> {
    ...
    const user: User = {
      ...
      complianceNotificationsEnabled:
        (existing as any)?.complianceNotificationsEnabled ?? true,
      notificationPreferences:
        (existing as any)?.notificationPreferences ?? {
          coiReminders: true,
          w9Reminders: true,
          contractReminders: true,
          leadEmails: true,
      ...
      complianceStatus: (existing as any)?.complianceStatus ?? "non_compliant",
      creditBalance: (existing as any)?.creditBalance ?? "0",
