...
Chromium and WebKit both passed the full shared workflow harness on Remodeling (32 scenarios per browser), Handyman (29) and Cabinet (26), plus seven administrator component widths each. Construction's generic fully-answered fixture now explicitly specifies no garage; previously it generated a valid extra garage question and then the harness incorrectly searched for Continue after navigating back. Back-navigation handling now supports returning through brand-specific questions. These are fixture/navigation assertions; the site's garage questions remain intact.

Construction's full shared Chromium/WebKit suite passed 29 scenarios in each engine plus all seven administrator widths. Its separate 14-case optional-address harness needed the current chip-then-Send-answer interaction and optional-email label. The harness now uses those controls and blocks non-fixture API/external requests. The optional-address scenarios remain a required workflow gate.
