 ...
 */
export const COST_CATALOG_VERSION = "2026.09.1";
...
const NEW_HOME_COMPONENTS: ComponentDef[] = [
  ...
  { id: "plumbing", label: "Plumbing", unit: "square foot", share: 0.035, quantity: { kind: "per-sqft", factor: 1.0 }, group: "direct", note: "Rough-in, water heater, gas lines, fixtures and trim." },
  { id: "electrical", label: "Electrical", unit: "square foot", share: 0.035, quantity: { kind: "per-sqft", factor: 1.0 }, group: "direct", note: "Panel, rough-in, devices, data and every light fixture." },
  ...
  { id: "hvac", label: "Heating and cooling", unit: "square foot", share: 0.03, quantity: { kind: "per-sqft", factor: 1.0 }, group: "direct", note: "Furnace, air conditioning, ducting, bath fans and the range hood run." },
  ...
  { id: "fixtures-hardware", label: "Fixtures, hardware and glass", unit: "square foot", share: 0.025, quantity: { kind: "per-sqft", factor: 1.0 }, group: "direct", note: "Door hardware, bath hardware, shower glass and mirrors." },
...
export function buildTakeoff(
  ...
  const components = getComponents(project).map((component) => {
    ...
    if (override !== undefined && Number.isFinite(override) && override >= 0) {
      return { ...component, unitCost: override };
    }
   ...
   * compressed to match the job actually being quoted - so a buyer reading the
   * breakdown sees proportions that came from real installed costs rather than
   * fixed percentages.
   */
