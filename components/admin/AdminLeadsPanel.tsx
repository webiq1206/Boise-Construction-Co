              ...
              {lineItems.map((item, index) => {
                ...
                return (
                  ...
                  >
                    <div className="flex justify-between items-center gap-2">
                      <div className="flex items-center gap-1.5 flex-wrap min-w-0">
                        ...
                        {(svcFreq !== "one-time" || (lead.frequency && lead.frequency !== "one-time")) && (
                          <Badge variant={recurring ? "default" : "secondary"} className="text-xs px-1 py-0">
                    ...
                    {item.description && (
                      <p className="text-xs text-muted-foreground mt-0.5" data-testid={`text-service-desc-${lead.id}-${index}`}>
                        {item.description}
                      </p>
                    )}
                    {measurement && (
                      <p className="text-xs text-muted-foreground mt-0.5" data-testid={`text-service-measurement-${lead.id}-${index}`}>
                        Measurement: {measurement}
                      </p>
                    )}
                    {item.calculationExplanation && (
                      <p className="text-xs text-muted-foreground/70 italic mt-1 pl-2 border-l-2 border-muted" data-testid={`text-service-explanation-${lead.id}-${index}`}>
                        {item.calculationExplanation}
                      </p>
                    )}
                  </div>
      ...
      <CardHeader className="pb-2">
        <div className="flex items-center justify-between gap-3">
          <div className="flex items-center gap-2 flex-wrap min-w-0 flex-1">
            ...
            {priority !== "normal" && (
              <Badge className={`${getPriorityColor(priority)} text-xs px-1.5 py-0`}>
                <Flag className="h-2.5 w-2.5 mr-0.5" />
                {priority.toUpperCase()}
              </Badge>
            )}
            <Badge variant={lead.status === "pending_admin" ? "default" : lead.status === "purchased" ? "secondary" : lead.status === "archived" ? "destructive" : "outline"} className="text-xs px-1.5 py-0" data-testid={`badge-status-${lead.id}`}>
            ...
            {lead.possibleDuplicates && lead.possibleDuplicates.length > 0 && (
              <HoverCard>
                <HoverCardTrigger asChild>
                  <Badge
                    variant="outline"
                    className="text-xs px-1.5 py-0 text-amber-700 border-amber-500 dark:text-amber-400 dark:border-amber-600 cursor-help"
                  ...
                  <ul className="space-y-2">
                    {lead.possibleDuplicates.slice(0, 5).map((d) => (
                      <li key={d.id} className="flex items-center justify-between gap-2 flex-wrap">
                        ...
                        <Badge variant="secondary" className="text-xs px-1 py-0">{d.status}</Badge>
                        {d.status !== "archived" && (
                          <AlertDialog>
            ...
            {lead.updatedAt && lead.createdAt && new Date(lead.updatedAt).getTime() - new Date(lead.createdAt).getTime() > 60000 && (
              <Badge
                variant="outline"
                className="text-xs px-1.5 py-0 text-blue-700 border-blue-500 dark:text-blue-400 dark:border-blue-600"
          ...
          <div className="flex items-center gap-1.5">
            ...
            {editingPrice ? (
              <div className="space-y-1.5 min-w-0">
                <div className="flex items-center gap-1.5">
                  <label className="text-xs text-muted-foreground w-16 flex-shrink-0">Estimate:</label>
                  <Input
                    type="number"
                    step="1"
                    min="0"
                ...
                <div className="flex items-center gap-1.5">
                  <label className="text-xs text-muted-foreground w-16 flex-shrink-0">Lead $:</label>
                  <Input
                    type="number"
                    step="1"
                    min="0"
