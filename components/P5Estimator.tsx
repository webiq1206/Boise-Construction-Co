...
export function P5Estimator({defaultService='',headingAs='h1',projectSource}:{defaultService?:string;headingAs?:'h1'|'h2';projectSource?:ProjectSource}){
  ...
  async function save(reviewed=false,clarification?:{id:string;answer:string}){
    ...
    const response=await operationFetch('/api/p5-estimator/draft',{method:'PUT',headers:{...draftHeaders(d),'Content-Type':'application/json'},body:JSON.stringify({text:d.text,answers:d.answers,contact:d.contact,revision:d.revision,wizard:d.wizard,reviewed,clarification})});
  ...
  async function analyze(){
    ...
    setBusy('Reading your documents and project details...');const form=new FormData();form.set('text',d.text);form.set('resumable','true');form.set('background','true');form.set('retry','true');
    let data:any;
    do{
    checkAnalysis();
  ...
  const projectInput=<>
    <div className={styles.field}><label htmlFor={`${id}-scope`}>Project details</label><textarea id={`${id}-scope`} rows={5} value={draft.text} onChange={e=>change({text:e.target.value})} placeholder={`${scopeExample}\nInclude any notes, instructions, inclusions or exclusions.`}/><p className={styles.hint}>Include dimensions, what to include or exclude, and who supplies materials.</p></div>
