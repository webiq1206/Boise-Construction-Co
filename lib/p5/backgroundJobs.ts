...
export async function queuedJob(input:Input,retry=false){
  ...
  const initial:Job={input,state:'queued',progress:input.kind==='analysis'?(input.draft.uploads.length?'Your complete document set is queued for analysis.':'Your project description is queued for analysis.'):'Your scope is queued for pricing.',attempts:0,createdAt:new Date().toISOString()};
    ...
    await Promise.all(rows.map(async row=>{
      ...
      try{
        ...
        if(job.input.kind==='analysis'){
          ...
          else{job.result=step;job.state='complete';job.progress=job.input.draft.uploads.length?'Document processing finished. Review the page coverage and any unreadable content.':'Project description analysis finished. Review the scope and any missing details.';}
        }else{
